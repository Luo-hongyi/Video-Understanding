...
def load_stories(story_pool: StoryPool, use_summary: False)->List[Document]:
    docs = []
    for segment_id in story_pool.roots.keys():
        for root_index in range(len(story_pool.roots[segment_id])):
            ...
            metadata = {"segment_id": segment_id, "root_index": root_index, "time": story_pool.roots[segment_id][root_index].event.current_time, "whole_story": story_line, "cumulative_particularity": story_pool.roots[segment_id][root_index].cumulative_particularity, "type": story_pool.roots[segment_id][root_index].event.event_type}
